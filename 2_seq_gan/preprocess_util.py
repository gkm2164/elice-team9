...
def get_sentence_pos(input):
    ...
    # save pkl
    _save_pickle('../data/real_data.pkl', data)
    _save_pickle('../data/pos_list.pkl', pos_list)

    # save txt
    f = open('../data/real_data.txt', 'w')
    for token in data:
        for word in token:
            word = str(word) + ' '
            f.write(word)
...
def get_preprocess_data(embedpath, data_pos_list):
    ...
    # save pkl
    _save_pickle('../data/pos2idx.pkl', pos2idx)
    _save_pickle('../data/idx2pos.pkl', idx2pos)
